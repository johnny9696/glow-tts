...
class TextMelLoader(torch.utils.data.Dataset):
    ...
    def get_text(self, text):
        ...
        if self.add_blank:
            text_norm = commons.intersperse(text_norm, len(symbols)+len(last_letter)) # add a blank token, whose id number is len(symbols)
...
class TextMelLangLoader(torch.utils.data.Dataset):
    ...
    def get_mel_text_speaker_pair(self, audiopath_sid_text):
        # separate filename, speaker_id and text
        audiopath, lang, text = audiopath_sid_text[0],audiopath_sid_text[2], audiopath_sid_text[3]
        text = self.get_text(text,lang)
        mel = self.get_mel(audiopath)
        lang=self.get_lang(lang)
        return (text, mel, lang)
...
class TextMelLangCollate():
    ...
    def __call__(self, batch):
        ...
        return text_padded, input_lengths, mel_padded, output_lengths, lang
...
class TextMelSpeakerLangCollate():
    ...
    def __call__(self, batch):
        ...
        return text_padded, input_lengths, mel_padded, output_lengths, sid, lang
