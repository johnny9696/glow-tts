...
from Speaker_Encoder.speaker_encoder import LSTM_Classification
import commons
import utils
...
def main():
  ...
  hps = utils.get_hparams(config='./config/single_lstm_class', model='LJ_LSTM_Classification')
  print(hps)
...
def train_and_eval(rank, n_gpus, hps):
  ...
  pre_model_path = "/media/caijb/data_drive/autoencoder/log/LSTM_Classification"
  LSTM_model = LSTM_Classification(input_size=hps.data.n_mel_channels, hidden_size=hps.LSTM.l_hidden, embedding_size=hps.LSTM.embedding_size,
  num_layers=hps.LSTM.num_layers, n_speaker=hps.model.n_speaker).to(device)

  LSTM_model,_,_,_= utils.load_checkpoint(utils.latest_checkpoint_path(pre_model_path,"G_*.pth"),LSTM_model)
  generator.emb_g.layer3_lstm = LSTM_model.layer3_lstm
  generator.emb_g.linear = LSTM_model.linear
