...
from Speaker_Encoder.speaker_encoder import Convolution_LSTM_classification
import commons
import utils
...
def train_and_eval(rank, n_gpus, hps):
  ...
  LSTM_model = Convolution_LSTM_classification(encoder_dim=hps.data.slice_length,
   ...
   , input_size=hps.data.n_mel_channels, embedding_size=hps.LSTM.embedding_size, 
   kernel=hps.LSTM.lstm_kernel, n_speaker=hps.model.n_speaker).to(device)

  LSTM_model,_,_,_= utils.load_checkpoint(utils.latest_checkpoint_path(pre_model_path,"G_*.pth"),LSTM_model)
  generator.emb_g.encoder = LSTM_model.encoder
  generator.emb_g.LSTM = LSTM_model.LSTM
